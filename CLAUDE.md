...
- **키 상태 사전 체크**(`/api/lapidary/status`): 드롭다운 열 때 provider별로 **토큰 0** 메타데이터
  호출(Anthropic `models.list`, Gemini `models`)로 키 설정·인증 여부만 확인 → 미설정/인증실패면
  옵션에 `키 없음/오류` 표기 + 경고 + `퇴고 시작` 비활성. **크레딧 잔액·유료 결제 활성은 무과금으로
  확인 불가**하므로(키는 유효해도 크레딧 0일 수 있음) 그건 실제 퇴고 요청 시 에러로만 드러난다.
